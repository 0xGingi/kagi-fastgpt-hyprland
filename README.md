...
4.  **Configure Toggle Script:**
    Edit `fastgpt/toggle_chat.sh` and set `PYTHON_SCRIPT_PATH` to the path of `fastgpt/main.py`. For example:
    ```bash
...
5.  **Configure Hyprland:**
    ...
    ```hyprland
    bind = $mainMod, X, exec, /path/to/your/fastgkagi-fastgpt-hyprlandpt/toggle_chat.sh
